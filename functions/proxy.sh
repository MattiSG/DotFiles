...
TUNNEL_REMOTE_PORT=443  # set this to a port on which you set up sshd to listen to on the proxy server; 443 is disabled by default, but using this port circumvents proxy prevention in countries that filter internet access, since traffic is disguised as HTTPS. See how to set up these ports on https://help.ubuntu.com/community/SSH/OpenSSH/Configuring#Troubleshooting.
